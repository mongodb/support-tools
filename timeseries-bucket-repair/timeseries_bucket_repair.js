...
//
// Helper function to validate namespaces, create temporary collection and return our timeseries
// options.
//
function verifyAndSetupCollsAndGetTSOptions(collName, tempColl) {
    ...
    if (tempRes && tempRes.length > 0) {
        ...
        if (tempOptions === undefined || tsOptions.timeField != tempOptions.timeField ||
...
//
function repairBucketByReinsertMeasurements(bucketId, collName, tempColl, tsOptions) {
    ...
    do {
        ...
        try {
            ...
            const sessionBucketColl =
                session.getDatabase(db.getName()).getCollection('system.buckets.' + collName);
...
// tempColl: String of a temporary namespace to use during the repair. This should be an unused
// namespace, if it exists its data will be lost.
//
